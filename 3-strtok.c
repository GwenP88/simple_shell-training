...
char **split_string(char *string)
{
	...
	if (string == NULL)
		return (NULL);
	/**
	 * Passage 1 (comptage): On duplique la chaîne
	 * car strtok() modifie le buffer
	 */
	...
	/**
	 * Comptage des mots: token pointe dans str1
	 * (buffer modifié). On incrémente count par token
	 */
	...
	/**
	 * Cas: uniquement des séparateurs => 0 token -
	 * On retourne un tableau vide (pas NULL)
	 */
	...
	/**
	 * Passage 2 (remplissage): Nouvelle copie car strtok()
	 * a consommé modifié la première
	 */
	...
	if (str2 == NULL)
		return (NULL);
	/**
	 * Allocation du tableau final: count mots + 1 case
	 * pour le NULL terminal
	 */
 ...
 * Return: pointer to a NULL-terminated array on success,
 * or NULL on failure
 */
